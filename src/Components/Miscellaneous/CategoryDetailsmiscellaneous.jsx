...
function CategoryDetailsmiscellaneous() {
  var category_from_storage = sessionStorage.getItem("categorymiscellaneous");
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:5000/miscellaneous/getCompleteData');
        if (isMounted) {
          setdata(response.data);
          console.log('Complete JSON Data:', response.data);
          console.log(" in travel Expense category details");
  
          if (response.data && response.data.miscellaneous && !response.data.miscellaneous[category_from_storage]) {
            sethaveNoData(true);
          }
        }
      } catch (error) {
  ...
  return (
    <div>
      {data &&
        Object.keys(data.miscellaneous).map((category) => {
          if (category === category_from_storage) {
           
            return Object.keys(data.miscellaneous[category]).map((year) => (
              <YearCard key={year} title={`Year ${year}`} content={data.miscellaneous[category][year]} dataisdeleted={()=>{ console.log("i am here ", refreshdata);setrefreshData(true)}} />
            ));
          }
          return null;
        })}
